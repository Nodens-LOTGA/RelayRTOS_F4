...
//################################################################################################################
bool ee24_write(uint16_t address, uint8_t *data, unsigned int len, uint32_t timeout) {
  if (ee24_lock == 1)
    return false;
  ee24_lock = 1;
  uint16_t w;
...
//################################################################################################################
bool ee24_read(uint16_t address, uint8_t *data, unsigned int len, uint32_t timeout) {
  if (ee24_lock == 1)
    return false;
  ee24_lock = 1;
#if (_EEPROM_USE_WP_PIN == 1)
